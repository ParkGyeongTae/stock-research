...
5. **본문에 원본 파일명을 노출하지 않는다** — 이 문서는 웹으로 발행되며, 독자는 `01_overview.md` 같은 파일명을 본 적도 없고 알 필요도 없다. 각 절 끝에 "→ 상세: `04_metrics.md`" 같은 줄이나 본문 중 "(`09_technical_daily.md` §3 참고)" 같은 괄호 인용을 넣지 않는다 — 다른 절을 가리켜야 하면 "위 §4에서 보듯"처럼 이 문서 자체의 절 번호로 참조한다. 원 문서로의 이동은 문서 하단 "관련 문서"·"참고 자료" 목록의 링크로만 제공한다.
