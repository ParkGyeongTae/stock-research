...
| 쓰지 않는 것 | 왜 |
|---|---|
| "최근", "최근 몇 년", "지금", "이 5년 동안", "지난 5년간" — **시장 상황·국면을 가리킬 때** | 재생성하면 가리키는 시점이 달라진다 |
...
첫 행의 시점 부사는 **지표가 무엇을 재는지 정의하는 자리에서는 예외**입니다 — "구리값은 지금 세계의 공장과 공사장이 얼마나 돌아가고 있는지에 가장 가까운 가격이다"의 "지금"은 이번 회차의 국면이 아니라 그 지표가 **언제 읽어도** 답해 주는 질문을 가리키므로 재생성해도 낡지 않습니다. 판정 기준은 단어가 아니라 문장입니다: **1년 뒤에 그대로 읽어도 참인가.** 참이면 두고, "지금은 완화 국면이다"처럼 그때의 상태를 단정하면 뺍니다.
...
`docs/macro/`의 단일 자산 문서(아래 표 17개)는 **1. 차트와 2. 해석만** 둔다 — 지지/저항 표·방법론 절은 2026-08-20에, 문서 하단의 관련 문서·참고 자료 목록은 2026-08-27에 걷어냈다. 그래서 각 문서 안에 재생성 커맨드를 반복해서 남기지 않는다. 아래 표가 전체의 티커·옵션에 대한 단일 출처다. 재생성할 땐 표의 값을 그대로 쓴다:
...
`--decimals 2`는 17개 문서 전부에 붙인다 — 기본 자동 규칙(20 이상이면 정수, 미만이면 소수 2자리)을 그대로 두면 지수·금 같은 큰 값에서 소수점이 사라져 기존 문서와 표시가 달라진다.
...
⚠️ **그래서 차트의 마지막 봉은 진행 중인 구간이다** — 종료일을 고정할 수단이 macro 문서에는 없다는 뜻이다(자세한 동작은 `scripts/gen_technical_chart.py` docstring "한계"가 마스터). 주중에 재생성하면 마지막 봉과 우측 "현재" 라벨이 그 주가 끝나기 전 값을 가리키고 다음 회차에 달라지므로, **차트에서 읽은 값을 확정 종가로 산문이나 다른 문서에 옮겨 적지 않는다** — 수치를 산문에 두지 않는 위 규칙을 지키면 저절로 막힌다. 문서마다 마지막 봉 날짜가 며칠씩 어긋나는 것도 같은 이유다(시장별 마감 시각·휴장일 차이).
