...
::: info
[CPI](./cpi.md)·[Core CPI](./core_cpi.md)·[Core PCE](./core_pce.md)의 전년동월비를 한 차트에 겹쳐, 세 지표가 어떻게 벌어지고 좁혀지는지 보기 위한 참고 자료다. [통화 3종 비교](../foreign_exchange/comparison.md)와 달리 **지수화하지 않는다** — 셋 다 이미 같은 단위(%)라 원값을 그대로 겹치는 편이 수준과 격차를 왜곡 없이 보여준다. 개별 지표의 산출 방식과 한계는 각 문서를 따로 참고할 것 — 이 문서는 그 셋을 대체하지 않는다.
...
- **CPI와 Core CPI의 격차**: 헤드라인에서 근원을 뺀 차이는 대체로 **에너지·식품이 물가에 얼마나 기여하고 있는지**를 나타낸다. CPI가 Core CPI 위로 벌어지면 유가·식료품이 밀어 올리는 국면, 아래로 내려가면 그 둘이 물가를 끌어내리는 국면으로 읽는다.
- **Core CPI와 Core PCE의 격차**: 둘 다 "에너지·식품 제외"인데도 숫자가 다른 이유는 물가가 달라서가 아니라 **바스켓 가중치와 포함 범위가 다르기 때문**이다(상세는 [Core PCE](./core_pce.md) 문서). 그래서 이 격차는 대체로 일정한 방향으로 유지되며, 정책 판단에 쓰는 기준은 연준이 목표를 매기는 Core PCE 쪽이다.
- **한계**: 셋 다 전년동월비라 기저효과를 공유한다 — 작년 같은 달의 수치가 특이했다면 세 선이 동시에 같은 방향으로 움직일 수 있는데, 이는 올해 물가의 변화가 아니라 비교 대상의 성질이다. 또 Core PCE는 나머지 둘보다 발표가 한 달가량 늦어, 차트 오른쪽 끝에서 선의 길이가 다를 수 있다.
