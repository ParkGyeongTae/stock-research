...
::: info
개인소비지출(PCE) 물가지수에서 에너지·식품을 뺀 전년동월비 상승률이다(FRED `PCEPILFE`). **연준이 2% 물가안정 목표를 매기는 기준이 바로 이 지표**라, 정책 방향을 가늠할 때는 [CPI](./cpi.md)보다 이쪽을 본다.
:::
...
- **한 줄로**: 연준이 "2%"라고 말할 때 실제로 보고 있는 숫자다.
- **오르면**: 목표에서 멀어지고 있다는 뜻으로, 긴축을 유지하거나 강화할 근거로 흔히 해석한다.
- **내리면**: 목표에 가까워지고 있다는 뜻으로, 금리인하 경로를 여는 신호로 흔히 해석한다.
- **왜 CPI와 숫자가 다른가**: 물가가 달라서가 아니라 만드는 방식이 다르다. ① **가중치** — PCE는 실제 지출 비중을 매 분기 갱신해 비싼 품목을 싼 품목으로 갈아타는 대체효과를 반영하지만, CPI는 상대적으로 고정된 바스켓을 쓴다. ② **포함 범위** — PCE에는 고용주가 내는 건강보험료나 정부가 부담하는 의료비처럼 **가계가 직접 내지 않는 지출**까지 들어간다. ③ 그 결과 의료비 비중은 CPI보다 크고 주거비 비중은 작다. 이 때문에 Core PCE는 통상 [Core CPI](./core_cpi.md)보다 낮게 나오는 경향이 있어, 두 숫자를 같은 잣대로 비교하면 안 된다.
- **속보성이 떨어진다**: CPI보다 발표가 늦고, 소매판매·CPI·PPI 등 다른 통계를 재료로 만들기 때문에 그 원자료가 개정되면 함께 소급 수정된다.
