# CEO / 경영진

> 대학 졸업장 없이 견습 엔지니어로 시작해 2006년 뉴질랜드에서 로켓랩을 창업한 독학 엔지니어. "우리는 돈이 없어서 머리를 써야 한다"는 원칙 아래 저비용 구조로 회사를 나스닥 상장사로 키워낸, 종종 "일론 머스크의 대척점"으로 묘사되는 CEO.

- **이름**: Sir Peter Beck (피터 벡)
- **직책**: President, Chairman & CEO
- **취임일**: 2006년 창업 이후 줄곧 CEO (2026년 기준 20년째)
- **출생년도 / 국적**: 1976~1977년경 생, 뉴질랜드 인버카길(Invercargill) 출신
- **학력**: 대학 진학 없이 1993년부터 현장에서 정밀공학을 익힌 독학(self-taught) 엔지니어
- **전임 CEO**: 없음 (창업자 겸 CEO)

---

## 1. 커리어 이력

| 기간 | 직책 / 회사 | 내용 |
|------|-------------|------|
| 1993년 | Fisher & Paykel (뉴질랜드 가전업체) | 견습 정밀엔지니어로 입사, 이후 생산기계·제품 설계 및 해석 업무로 확장 |
| 2003년 | Industrial Research Limited (뉴질랜드 정부 연구기관) | 고성능 복합소재 구조 연구, 풍력터빈·초전도체 등 엔지니어링 프로그램 주도 |
| 2006년 | Rocket Lab 창업 (뉴질랜드) | 소형 위성 발사체 개발 시작 |
| 2009년 | Ātea-1 발사 | 남반구 최초의 민간 개발 로켓으로 우주 도달 |
| 2013년 | 본사 미국 이전 | 캘리포니아 헌팅턴비치(이후 2020년 롱비치로 재이전) |
| 2017년 | Electron 첫 궤도 발사 시도 | 이후 상업 발사 서비스 본격화 |
| 2021년 8월 | 나스닥 상장 | Vector Acquisition Corp과의 SPAC 합병을 통해 RKLB로 상장 |
| 2026년 6월 | Iridium Communications 인수 계약 발표 | 80억 달러(EV) 규모 현금+주식 인수. 완료 시 발사·위성 제조·위성통신을 아우르는 구조로 확장(개요 참고) |

영국 왕실로부터 기사 작위(Knight Bachelor)를 받아 "Sir Peter Beck"으로 불림.

---

## 2. 경영 스타일 / 전략 방향

- **"엔드투엔드 우주기업" 비전**: 발사 서비스(Electron/Neutron)뿐 아니라 위성 설계·제조·부품 공급·궤도상 운영까지 아우르는 수직계열화 전략을 지속적으로 추진. Neutron을 "마지막 퍼즐 조각"으로 표현하며 이를 통해 완전한 엔드투엔드 우주기업 구조를 완성하려 함.
- **저비용·실용주의 문화**: "우리는 돈이 없어서 머리를 써야 한다(We have no money, so we have to think)"는 발언으로 대표되는 검소한 엔지니어링 문화를 강조. 스스로 대학을 나오지 않은 독학 엔지니어 출신이라는 점이 실행 중심 문화에 영향을 미친 것으로 평가됨.
- **SpaceX와의 정면 경쟁 선언**: 중형 재사용 로켓 Neutron 개발 목적을 "50%는 중형 발사 시장의 독점(사실상 SpaceX Falcon 9)을 깨는 것, 50%는 자사 위성을 직접 발사하는 것"이라고 명시하며 SpaceX에 대한 정면 도전 의사를 공개적으로 밝힘.

---

## 3. 주요 성과 / 논란

- **성과**: Electron을 미국에서 SpaceX Falcon 9 다음으로 발사 빈도가 높은 로켓으로 성장시킴. 2021년 SPAC 합병을 통한 나스닥 상장 이후 Space Systems 사업을 확장해 위성 제조·부품 사업을 회사의 최대 매출원으로 키움.
- **논란 / 리스크**:
  - **Neutron 개발 지연·기술적 난관**: 중형 재사용 로켓 Neutron 시험 과정에서 연료탱크 파열 사고가 발생하는 등 개발 일정 지연 우려가 제기됨.
  - **현금 소진(cash burn) 우려**: 공매도 리서치 기관 Bleecker Street Research 등은 "Rocket Lab의 현금 소진이 수년간 멈추지 않을 것"이라며 Neutron 상업화 이전 추가 자금 조달이 필요할 수 있다는 비판적 분석을 제기 — 이는 공매도 진영의 견해로 별도 검증 필요.
  - **최근 대규모 내부자 매도**: 2026년 7월 6~8일 Beck 관련 가족신탁(Equatorial Trust)이 사전 예정된 10b5-1 매매 계획에 따라 주당 $81.59~$101.57 가격으로 약 327.6만 주(약 2.864억 달러 상당)를 매도했고, Iridium 인수 발표 직후에는 추가로 약 500만 주(약 4.65억 달러 상당) 매도 계획을 신고했다. 10b5-1 계획에 따른 사전 예정 매도라 재량적 매도보다는 덜 부정적으로 해석될 여지가 있으나, 4. 보상 / 지분의 2025년 자진 보상 삭감·RSU 반납 행보와는 대조적인 최근 행보라 함께 짚어둘 필요가 있음.

---

## 4. 보상 / 지분

| 항목 | 값 | 기준연도 |
|------|-----|----------|
| 총보상 (Total Compensation) | 약 680만 달러 (기본급 11.7% + 주식·보너스 88.3%), 전년 2,010만 달러 대비 대폭 감소 | FY2025 |
| 보상 구조 변경 | 2025년 프록시 공시 이후 기본급을 연 80만 달러 → 1달러로 자진 삭감하고, 연간 보너스 수령 권리를 포기. 보유 RSU 392,155주(약 2,250만 달러 상당)도 자진 반납·소각 | 2025년 발표 |
| 회사 지분율 (%) | 약 0.37% (약 1억 7,960만 달러 상당) | 최근 공시 기준 |

---

## 5. 시장 / 투자자 평가

애널리스트 18곳 컨센서스 기준 "매수(Buy)" 의견이 우세하며 평균 목표주가는 약 111.31달러 수준. Neutron을 통한 SpaceX 독점 구도 타파 스토리에 대한 시장 기대가 주가 상승을 견인해온 반면, 공매도 진영을 중심으로 현금 소진 속도와 Neutron 상업화 시점에 대한 회의적 시각도 공존.

---

## 6. 지분 구조 / 거버넌스

| 항목 | 값 | 기준일 |
|------|-----|--------|
| 기관 투자자 지분율 (%) | 약 35~72% (출처별 편차 큼 — fintel/Business Quant/TipRanks 등 집계 방식 차이로 추정, 확인 필요) | 2026년 상반기 기준, 출처마다 조회 시점 상이 |
| 내부자(임원·이사) 지분율 (%) (4. 보상 / 지분의 CEO 개인 지분 포함 합계) | 약 0.88% | 2026년 상반기 기준(집계 사이트) |
| 주요 대주주 (5% 이상 보유) | 확인 필요 (기관 지분이 여러 자산운용사에 분산돼 있어 단독 5% 이상 보유자는 별도 확인 필요) | — |
| 차등의결권 구조 (dual-class) | 없음으로 파악됨 — 단일 클래스 보통주(1주 1표) 체제로 보이며, [SpaceX](../spacex/01_overview.md)와 달리 별도 차등의결권 공시는 확인되지 않음. 다만 10-K 원문 대조는 하지 못해 확인 필요로 남김 | — |

---

## 관련 문서

- [개요](./01_overview.md)
- [역사 / 주요 이벤트](./02_history.md)
- [핵심 지표](./04_metrics.md)
- [재무 / 실적](./05_financials.md)
- [밸류에이션 / 적정주가](./06_valuation.md)
- [투자 판단](./07_investment.md)
- [최근 뉴스 / 이슈](./08_news.md)

---

## 참고 자료

- [Peter Beck - Wikipedia](https://en.wikipedia.org/wiki/Peter_Beck)
- [How Rocket Lab CEO Peter Beck built a multibillion-dollar company (CNBC)](https://www.cnbc.com/2023/04/11/how-rocket-lab-ceo-peter-beck-built-multibillion-dollar-company.html)
- [Rocket Lab CEO: 'We have the right combination' to break SpaceX's monopoly (AOL/Yahoo Finance)](https://www.aol.com/finance/rocket-lab-ceo-combination-break-194214721.html)
- [Rocket Lab CEO Reduces Pay to $1 and Forfeits Major RSU Holdings (MLQ News)](https://mlq.ai/news/rocket-lab-ceo-reduces-pay-to-1-and-forfeits-major-rsu-holdings/)
- [Peter Beck salary at Rocket Lab Corp (Salary.com)](https://www.salary.com/research/executive-compensation/peter-beck-executive-member-of-rocket-lab-usa-inc)
- [Rocket Lab (RKLB): We Think It's Gonna Be a Long, Long Time (Bleecker Street Research)](https://www.bleeckerstreetresearch.com/research/rklb)
- [Rocket Lab Stock Forecast & Analyst Price Targets (StockAnalysis)](https://stockanalysis.com/stocks/rklb/forecast/)
- [Rocket Lab trust sells 3,275,779 shares (StockTitan Form 4)](https://www.stocktitan.net/sec-filings/RKLB/form-4-rocket-lab-corp-insider-trading-activity-88b792454463.html)
- [RKLB Stock Slips Overnight: CEO Peter Beck Eyes Trimming Stake By $465M After Iridium Deal (Yahoo Finance)](https://finance.yahoo.com/markets/stocks/articles/rklb-stock-slips-overnight-ceo-042842410.html)

---

*작성일: 2026-08-08 (최종 수정일: 2026-08-23)*
