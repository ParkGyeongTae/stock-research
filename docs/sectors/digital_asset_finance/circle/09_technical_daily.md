...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, Yahoo Finance 일봉 API에서 직접 수집했다(1년 일봉은 `04_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). **겹치는 시점의 종가 대조(이 문서 수집 시점 2026-08-16 기준)**: 2026-08-13 종가 $75.38은 당시 `04_metrics.md` A.2·`06_valuation.md`가 인용한 stockanalysis.com 기준값과 정확히 일치했다. FY2025 회계연도 말(2025-12-31) 종가 $79.30 역시 `04_metrics.md` A.2 "FY2025" 열 값과 정확히 일치한다. ⚠️ **이후 갱신 참고**: `04_metrics.md`·`06_valuation.md`는 2026-08-19~21 급등을 반영해 2026-08-21 종가($87.98) 기준으로 갱신됐으나(2026-08-22), 이 일봉 차트 자체는 아직 2026-08-14까지의 데이터만 반영한다 — 차트를 최신 가격까지 재생성하기 전까지는 이 문서의 "현재가"($71.60)가 다른 문서보다 며칠 뒤처져 있다는 점에 유의할 것.
>
