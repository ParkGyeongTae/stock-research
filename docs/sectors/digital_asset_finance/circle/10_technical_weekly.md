...
상장 후 69주치뿐이고 그 안에서 주가가 $49.90~$159.47로 3.2배 움직여, **터치 2회 이상인 클러스터가 $138 하나밖에 검출되지 않았다.** **현재가 아래쪽에 레벨이 없는 것을 신저가 구간으로 읽지 말 것** — 아래쪽에 스윙 저점은 있고(가장 먼 것이 $50으로 현재가보다 41% 아래), 터치 2회 조건을 넘긴 묶음이 없었을 뿐이다. **이 문서는 참고 자료로만 쓰고, 실질적인 레벨 판단은 [일봉 차트](./09_technical_daily.md)에 의존한다.**
