...
>
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, Yahoo Finance 주봉 API에서 직접 수집했다. 두 문서에서 겹치는 시점의 종가를 대조한 결과: 2026-08-14 종가 $71.60은 [`09_technical.md`](./09_technical.md)의 동일 시점 종가와 일치(같은 스크립트·같은 원자료 출처).
...
| R1 | $138 | 2 | 2026년 3월·5월(2026-03-16·2026-05-11) 스윙 고점대 — `09_technical.md`의 레벨과 함께 최근 레짐의 상단을 보여줌 |
...
| 참고선 | $299 | — | 상장 후 최고가(2025-06-23). 이후 두 차례(§3, `09_technical.md` §3)의 대형 조정을 거치며 현재가와 4배 이상 괴리돼 있어 근시일 저항으로 보지 않음 |
...
- 이후 2026년 5월(주간 고점 $140.00)까지 반등했으나, `09_technical.md` §3이 다루는 2026-06-30 OUSD 컨소시엄 출범·지수 제외 겹친 급락으로 다시 큰 폭 조정받았다(주간 저점 $57.84, 2026-08-03, 고점 대비 -58.7%) — **상장 후 14개월 사이에 이미 두 차례의 대형 조정(-83.3%, -58.7%)을 겪은 셈**이다. 이는 회사 펀더멘털의 구조적 문제라기보다, 신규 상장·초기 유통물량이 적은 종목 특유의 높은 변동성과 두 차례의 서로 다른 계기(자체 재평가, 경쟁 컨소시엄 출범)가 겹친 결과로 해석하는 것이 합리적이다.
- 두 번째 조정(2026-06-30 이후)의 상세 내용은 `09_technical.md` §3에 이미 기록돼 있어 여기서 중복 서술하지 않는다 — 이 문서(§3)는 일봉 문서가 다루지 않는 첫 번째 조정(상장~2026년 2월)에 초점을 맞춘다.

---

## 4. 방법론 · 한계

- **데이터**: Yahoo Finance 주봉 OHLCV(주간 시가/고가/저가/종가/거래량, 주 마지막 거래일 기준), 64개 주, 2025-06-02~2026-08-14. 수집 시점: 2026-08-16. 원주가(과거 분할은 소급 반영, 배당은 미반영) — Circle은 이 구간 내 일반적 의미의 주식분할·병합 이력이 없다(`04_metrics.md` 상단 경고·`09_technical.md` §4와 동일 확인). IPO 시 우선주의 보통주 전환·Class B 신설은 스톡스플릿이 아니므로 소급조정 대상이 아니다.
...
- **스윙 포인트 탐지**: 각 주의 고가/저가가 전후 4주(총 9주 창) 내 최고/최저값과 같으면 스윙 고점/저점으로 분류. `09_technical.md`(전후 5거래일)보다 창이 넓다.
...
- **한계**:
    ...
    - `09_technical.md` §3의 2026-06-30 이후 급락처럼 뉴스 이벤트로 인한 불연속 구간은 통상적인 지지/저항 해석이 적용되기 어렵다.
