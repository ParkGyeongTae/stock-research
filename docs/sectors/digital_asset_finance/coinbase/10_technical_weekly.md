...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 Yahoo Finance 주봉 API에서 직접 수집했다(5년 주봉은 `04_metrics.md`가 다루는 범위 밖). 두 문서에서 겹치는 시점의 종가를 대조한 결과: 2026-08-14 종가 $148.47은 [`09_technical.md`](./09_technical.md)의 동일 시점 종가와 일치(같은 스크립트·같은 원자료 출처).
>
...
| R1 | $181 | 1 | 2026년 7월(2026-07-20, Q2 실적발표 직전) 스윙 고점. `09_technical.md`의 R1($175)과 근접한 레벨 — 터치 1회지만 가장 최근 레짐의 저항으로 참고 |
...
| 참고선 | $445 | — | 5년 최고(2025-07-14). 2026-07-31 Q2 실적발표 갭다운(`09_technical.md` §3) 이후 레짐이 재설정됐고, 애초에 현재가 대비 3배 가까이 괴리돼 있어 근시일 저항으로 보지 않음 |
...
- 이 폭락·회복 사이클은 회사 고유 이슈보다 크립토 자산 가격 전반의 사이클(비트코인·이더리움 등)에 연동된 것에 가깝다 — `01_overview.md`·`07_investment.md`가 짚는 "크립토 시장 구조적 변동성 노출" 리스크가 가격에 그대로 나타난 역사적 사례로 볼 수 있다. `09_technical.md` §3의 2026-07-31 실적발표 갭다운(회사 고유·최근 이벤트)과는 시점·원인이 다르므로 혼동하지 말 것.

---

## 4. 방법론 · 한계

- **데이터**: Yahoo Finance 주봉 OHLCV(주간 시가/고가/저가/종가/거래량, 주 마지막 거래일 기준), 262개 주, 2021-08-16~2026-08-14. 수집 시점: 2026-08-16. 원주가(과거 분할은 소급 반영, 배당은 미반영) — Coinbase는 2021년 상장 이후 분할·병합 이력이 없다(`02_history.md`·`09_technical.md` §4와 동일 확인). 무배당 기업이라 배당 관련 조정 이슈 자체가 없다(`04_metrics.md` A.4).
...
- **한계**:
    ...
    - `09_technical.md` §3의 2026-07-31 실적발표 갭다운처럼 뉴스 이벤트로 인한 불연속 구간은 통상적인 지지/저항 해석이 적용되기 어렵다.
