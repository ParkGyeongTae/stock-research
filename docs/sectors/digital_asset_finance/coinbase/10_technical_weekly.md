...
5년 구간에서 **터치 2회 이상인 클러스터가 $140 하나뿐**이다. Coinbase 주가가 2021년 상장 후 $30대에서 $400대까지 10배 폭으로 움직여, ±2.5% 허용오차로는 스윙 포인트가 거의 묶이지 않기 때문이다. **현재가 위쪽에 레벨이 없는 것을 신고가 구간으로 읽지 말 것** — 위쪽에 스윙 고점은 얼마든지 있고(가장 먼 것이 $445로 현재가의 2.6배), 터치 2회 조건을 넘긴 묶음이 없었을 뿐이다.
