# Coinbase (코인베이스) — 기술적 분석 (주봉 캔들차트 · 5년 지지/저항)
...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 Yahoo Finance 주봉 API에서 직접 수집했다(5년 주봉은 `04_metrics.md`가 다루는 범위 밖). 두 문서에서 겹치는 시점의 종가를 대조한 결과: 2026-08-14 종가 $148.47은 [`09_technical_daily.md`](./09_technical_daily.md)의 동일 시점 종가와 일치(같은 스크립트·같은 원자료 출처).
>
> ⚠️ **자연 클러스터가 극히 희소한 종목**: Coinbase는 5년간 주가 변동폭이 극단적으로 커서(최저 $31.55~최고 $444.65, 약 14배 차이) 스윙 고점·저점이 ±2.5% 이내로 반복되는 경우가 매우 드물다. 실제로 이 표준 파라미터(전후 4주·±2.5%)로는 터치 2회 이상인 지지선이 S1(아래 §2) 하나뿐이었다 — 나머지 레벨은 대부분 터치 1회(예외 포함)이며, 이는 데이터·방법론의 오류가 아니라 이 종목 고유의 높은 변동성을 그대로 반영한 결과다.
...
@media (prefers-color-scheme: dark) {
  .coin-chart { --bg:#1a1a19; --grid:#2c2c2a; --axis:#383835; --ink:#ffffff; --ink2:#c3c2b7; --muted:#898781; --up:#0ca30c; --down:#e66767; --support:#3987e5; --resistance:#d95926; --ref:#898781; }
}
...
| R1 | $181 | 1 | 2026년 7월(2026-07-20, Q2 실적발표 직전) 스윙 고점. `09_technical_daily.md`의 R1($175)과 근접한 레벨 — 터치 1회지만 가장 최근 레짐의 저항으로 참고 |
| **현재가** | **$148.47** (2026-08-14 종가) | — | R1과 S1 사이 |
| S1 | $140 | 3 | 2025년 4월·2026년 2월·6월(2025-04-07·2026-02-09·2026-06-22)에 반복된 저점대 — 이 종목에서 유일하게 자연적으로 형성된 2회 이상 터치 레벨(§4 한계 참고) |
| S2 | $32 | 1 | 2023-01-02 크립토 겨울 저점(§3 참고). 터치 1회지만 5년 최저치이자 구조적 전환점이라 예외 포함 |
| 참고선 | $445 | — | 5년 최고(2025-07-14). 2026-07-31 Q2 실적발표 갭다운(`09_technical_daily.md` §3) 이후 레짐이 재설정됐고, 애초에 현재가 대비 3배 가까이 괴리돼 있어 근시일 저항으로 보지 않음 |

> 표시 기준은 "현재가에서 가까운 순으로 각 방향 3개"다(§4 생성 스크립트 기본값). 위 상단 경고에서 밝혔듯 이 종목은 자연 클러스터(터치 2회 이상)가 S1 하나뿐이라, 나머지 4개 레벨(R1·R2·S2·참고선)은 전부 `--force-level`·`--ref-line`으로 강제 포함한 예외다 — 사유는 각 행에 명시했다.
...
- 이후 회복은 매우 더뎠다 — 2023-01-02 저점 형성 후 하락 전 고점($368.90)을 다시 상회하기까지 약 2.5년(2025-07-07)이 걸렸다. S&P500 편입(2025-05-19, `02_history.md`)과 2025년 크립토 강세장이 겹치며 뒤늦게 완전히 회복된 흐름이다.
- 이 폭락·회복 사이클은 회사 고유 이슈보다 크립토 자산 가격 전반의 사이클(비트코인·이더리움 등)에 연동된 것에 가깝다 — `01_overview.md`·`07_investment.md`가 짚는 "크립토 시장 구조적 변동성 노출" 리스크가 가격에 그대로 나타난 역사적 사례로 볼 수 있다. `09_technical_daily.md` §3의 2026-07-31 실적발표 갭다운(회사 고유·최근 이벤트)과는 시점·원인이 다르므로 혼동하지 말 것.

---

## 4. 방법론 · 한계

- **데이터**: Yahoo Finance 주봉 OHLCV(주간 시가/고가/저가/종가/거래량, 주 마지막 거래일 기준), 262개 주, 2021-08-16~2026-08-14. 수집 시점: 2026-08-16. 원주가(과거 분할은 소급 반영, 배당은 미반영) — Coinbase는 2021년 상장 이후 분할·병합 이력이 없다(`02_history.md`·`09_technical_daily.md` §4와 동일 확인). 무배당 기업이라 배당 관련 조정 이슈 자체가 없다(`04_metrics.md` A.4).
...
- **한계**:
    ...
    - **이 종목은 다른 회사 문서 대비 유독 방법론의 한계가 두드러진다** — 위 상단 경고대로 자연 클러스터가 S1 하나뿐이라 §2 레벨 대부분이 강제 포함 예외이고, 이는 스윙 탐지·클러스터링 방법론 자체가 극단적 고변동성 종목에는 잘 들어맞지 않는다는 뜻이다. 지지/저항 레벨을 다른 회사 문서와 같은 신뢰도로 취급하지 말 것.
    - `09_technical_daily.md` §3의 2026-07-31 실적발표 갭다운처럼 뉴스 이벤트로 인한 불연속 구간은 통상적인 지지/저항 해석이 적용되기 어렵다.
