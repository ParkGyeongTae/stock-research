...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, Yahoo Finance 일봉 API에서 직접 수집했다(1년 일봉은 `04_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). **겹치는 시점의 종가 대조**: 2026-08-13 종가 $153.90은 `04_metrics.md` A.2·`06_valuation.md`에서 인용한 stockanalysis.com 기준값($153.90)과 정확히 일치한다. FY2025 회계연도 말(2025-12-31) 종가 $226.14 역시 `04_metrics.md` A.2 "FY2025" 열 값과 정확히 일치한다.
