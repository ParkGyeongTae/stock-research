# BWX Technologies (BWX 테크놀로지스) — 기술적 분석 (일봉 캔들차트 · 지지/저항)
...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, Yahoo Finance 일봉 API에서 직접 수집한 것이다(1년 일봉은 `04_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). **겹치는 시점 종가 대조**: 2026-08-17 종가는 이 문서 기준 $172.89(Yahoo Finance), `04_metrics.md`·`06_valuation.md`가 인용한 stockanalysis.com 기준 $172.86과 $0.03(약 0.02%) 차이로 사실상 일치 — 데이터 제공처 간 반올림·마감가 집계 시점 차이로 판단되며, 수정주가 여부를 다시 확인할 필요는 없는 수준이다.
...
| 참고선 | $242 | — | 52주 최고(정확히는 $241.82) — 이후 밸류에이션 부담·모멘텀 둔화로 조정에 들어가 현재는 이 레벨과 단절된 국면(§3·`07_investment.md` §3 리스크1 참고), 근시일 저항으로 보지 않음 |
...
- 2026년 상반기 중 52주 최고 $241.82를 기록한 뒤, 밸류에이션 부담과 단기 모멘텀 둔화로 최근 90일 기준 약 −27%의 조정을 거쳐 현재 $172.89 수준까지 되돌렸다(관련 코멘터리는 [`08_news.md`](./08_news.md) 2026-08-03 Q2 실적·가이던스 상향 항목 참고 — 실적·가이던스 자체는 견조했음에도 밸류에이션 재평가가 함께 진행된 것으로 보인다).
...
- 이 되돌림 이후 거래 레짐은 R2($219)~S1($166) 박스권에 가깝게 재형성된 것으로 보이며, 52주 최고 부근 레벨은 현재로선 근시일 저항으로 작동하지 않는 것으로 판단해 §2에서 참고선으로 분리했다.
...
- **한계**:
    ...
    - §3에서 다룬 조정 구간은 점진적 되돌림이라 이 모델(스윙 고점/저점 빈도)이 포착하는 정보에 한계가 있다 — 왜 조정이 일어났는지(밸류에이션 재평가 등)는 이 기술적 모델로는 설명되지 않는다.
...
- [Yahoo Finance — BWXT 일봉 시세](https://finance.yahoo.com/quote/BWXT/history/) (수집 2026-08-18)
