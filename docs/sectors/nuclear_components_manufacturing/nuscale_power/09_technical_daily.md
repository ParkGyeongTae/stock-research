...
> ⚠️ 이 문서의 가격 데이터는 핵심 지표의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API에서 직접 수집한 것이다(1년 일봉은 핵심 지표가 다루는 범위 밖이라 단일 출처 규칙의 예외). **2026-08-18 종가 $8.64는 핵심 지표 A.2·밸류에이션 / 적정주가 §6에 인용된 stockanalysis.com 값과 일치한다.**
>
