# NuScale Power (SMR) — 기술적 분석 (주봉 캔들차트 · 다년 구조)
...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로 수집됐다(5년 주봉은 `04_metrics.md`가 다루는 범위 밖). **2026-08-18 종가 $8.64는 `04_metrics.md` A.2·`06_valuation.md` §6에 인용된 값과 일치한다.**
>
...
> $11~15 구간은 이 회사의 상장 이후 5년 내내 반복적으로 되돌아온 몇 안 되는 "안정적" 가격대다 — 다만 2022년(스팩 상장 초기)·2024년(랠리 전 바닥)·2026년(실적 충격 후 조정)이라는 **서로 완전히 다른 국면**에서 형성된 레벨이 우연히 겹친 것일 수 있어, 이 자체를 강한 구조적 지지/저항으로 과신하지 말 것(§4 한계 참고). 5년 중 상당 기간(2025년 8~11월) 주가가 이 구간을 크게 벗어나 있었다는 점도 함께 봐야 한다.
...
- 2025-11-06 3분기 실적 발표(Milestone Contribution $507.4M 비용 인식, `08_news.md` 참고)를 기점으로 급락이 시작돼 2026-08-18 기준 $8.64까지 되돌림 — 52주 최고 대비 약 −85%.
- 이 사건 전후로 거래 레짐이 완전히 바뀌었다 — 급등 구간(2025년 여름~10월)은 스윙 클러스터를 형성할 만큼 충분히 머물지 못해 §2 레벨 표에는 반영되지 않았고, 참고용 배경으로만 남긴다.
...
- **한계**:
    ...
    - 2025년 하반기 급등·급락(§3)으로 가격대가 두 차례 구조적으로 재설정됐다 — 5년 구간 전체를 하나의 연속된 레짐으로 해석하면 안 된다.
