...
> ⚠️ 이 문서의 가격 데이터는 핵심 지표의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API(Yahoo Finance)에서 직접 수집했다(1년 일봉은 핵심 지표가 다루는 범위 밖이라 단일 출처 규칙의 예외). 겹치는 시점의 종가를 대조한 결과: `2025-09-30` 종가 $341.38은 [핵심 지표](./04_metrics.md) A.2·[밸류에이션 / 적정주가](./06_valuation.md) §2에 인용된 FY2025 회계연도 말 종가와 **일치**한다. `2026-08-14` 종가 $364.15도 [개요](./01_overview.md)·[밸류에이션 / 적정주가](./06_valuation.md)에 인용된 현재주가와 **일치**한다.
