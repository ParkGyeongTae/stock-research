...
> ⚠️ 이 문서의 가격 데이터는 `05_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API(Yahoo Finance)에서 직접 수집했다(1년 일봉은 `05_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). 겹치는 시점의 종가를 대조한 결과: `2025-09-30` 종가 $341.38은 [`05_metrics.md`](./05_metrics.md) A.2·[`06_valuation.md`](./06_valuation.md) §2에 인용된 FY2025 회계연도 말 종가와 **일치**한다. `2026-08-14` 종가 $364.15도 [`01_overview.md`](./01_overview.md)·[`06_valuation.md`](./06_valuation.md)에 인용된 현재주가와 **일치**한다.
...
- 2026-04-28(화) 장 마감 후 Visa가 FY2026 2분기 실적을 발표(순매출 +17% YoY, GAAP EPS +36% YoY, Non-GAAP EPS +20% YoY) — 관련 원자료는 [`05_metrics.md`](./05_metrics.md) B절, 발표 사실 로그는 [`08_news.md`](./08_news.md) 참고.
