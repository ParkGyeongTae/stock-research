...
> ⚠️ 이 문서의 가격 데이터는 핵심 지표의 원자료 표와는 별도로, 이 차트 작성을 위해 주봉 API(Yahoo Finance)에서 직접 수집했다(5년 주봉은 핵심 지표가 다루는 범위 밖이라 단일 출처 규칙의 예외). 겹치는 시점의 종가를 대조한 결과: `2026-08-14` 종가 $364.15는 [개요](./01_overview.md)·[핵심 지표](./04_metrics.md) A.2·[밸류에이션 / 적정주가](./06_valuation.md)에 인용된 현재주가와 **일치**한다. 이 주봉 데이터의 주 경계(월요일 기준)는 기술적 분석 — 일봉·1년·핵심 지표가 쓰는 회계연도 말(9/30) 종가와 정확히 같은 날짜로 떨어지지 않아 별도로 대조하지 않았다.
...
- 2024-09-23(월) 장 마감 후 언론 보도로 미 법무부(DOJ)의 소송 준비 소식이 알려졌고, 2024-09-24(화) DOJ가 Visa를 상대로 직불카드(debit) 시장 독점 혐의 반독점 소송을 정식 제기했다. 관련 항목은 [최근 뉴스 / 이슈](./08_news.md)에 아직 로그로 남아 있지 않음(2024년 항목은 4분기 경과로 아카이브 대상) — 소송 자체는 [투자 판단](./07_investment.md) 3. 리스크 (약점 / Bear Case) "반독점 소송" 리스크 서술에 포함.
...
- 이후 주가는 2025년 들어 완만히 회복해 2025년 3월·6월 R1 구간($366~$376)까지 올라섰다 — DOJ 소송이라는 구조적 리스크가 상존함에도 실적 성장이 주가 회복을 견인한 패턴으로 해석된다(펀더멘털 판단은 [밸류에이션 / 적정주가](./06_valuation.md)·[투자 판단](./07_investment.md) 참고, 이 문서는 가격 패턴 서술에 한정).
...
- **한계**:
    ...
    - 5년 구간 안에 사업 구조 자체가 바뀐 대형 M&A·사업부 매각·상장 형태 변경은 없었다 — 다만 2024-09 DOJ 소송(3. 관측된 특이 구간 — 2024-09-23(주간) DOJ 반독점 소송(debit 시장 독점) 제기)이라는 구조적 법률 리스크 발생은 있었고, 그 이전 스윙 레벨(2024년 3~9월 저항대 $280~$293)은 이 사건 이후 레짐과 단절돼 2. 지지선 / 저항선 요약에서 참고 레벨로도 포함하지 않았다(터치 부족으로 자연 제외).
...
- [Yahoo Finance — Visa Inc. (V) 주봉 시세](https://finance.yahoo.com/quote/V/history/)
