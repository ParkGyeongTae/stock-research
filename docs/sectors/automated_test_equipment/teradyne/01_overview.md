...
- **시가총액**: 약 $572.9억 (2026-08-25 종가 $366.43, 발행주식수 약 156.34M주 기준 — 출처: stockanalysis.com)
...
Teradyne은 Advantest와 함께 반도체 테스트 장비(ATE) 시장을 양분하는 기업으로,
**SoC(System-on-Chip) 테스트**에서 강세를 보인다. 최근에는 협동로봇(Universal Robots),
자율이동로봇(MiR) 등 **Robotics** 사업으로 포트폴리오를 다각화해, 반도체 사이클과
무관한 별도 성장축을 함께 갖추고 있다.
...
ATE(반도체 테스트 장비) 시장은 **Teradyne과 Advantest 두 회사가 사실상 양분**하는 듀오폴리 구조다.

- **높은 진입장벽 + 듀오폴리 구조**: 고객(반도체 회사·OSAT)의 신뢰성 검증 사이클이 길어 전환비용이 매우 높음.
- **AI 반도체 수혜**: 2026년 2분기 컴퓨트 관련 SoC 매출이 전년 대비 약 600% 성장할 만큼, GPU·AI 가속기 복잡도 증가가 테스트 수요를 직접 견인. AI 관련 매출이 전체 매출의 60% 이상을 차지.
- **Robotics 사업의 분산 효과**: 아직 매출 비중은 작지만(약 7%), 반도체 경기와 상관관계가 낮은 별도 성장축으로 포트폴리오 리스크를 분산.
