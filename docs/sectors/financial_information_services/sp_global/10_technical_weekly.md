# S&P Global (에스앤피 글로벌) — 기술적 분석 (주봉 캔들차트 · 지지/저항)

> 최근 5년 주봉 가격 흐름을 지지선·저항선과 함께 정리한 기술적(가격 패턴) 참고 자료 — 여러 금리·경기 사이클을 관통하는 다년 구조를 보기 위한 것이다. 최근 1년 세부 흐름은 [`09_technical_daily.md`](./09_technical_daily.md)(일봉·1년)를 참고. **이 문서는 과거 가격 패턴에 대한 객관적 서술이며, 매수/매도 신호나 목표가 예측이 아니다** — 펀더멘털 기반 적정주가 판단은 [`06_valuation.md`](./06_valuation.md), 투자 결론은 [`07_investment.md`](./07_investment.md)를 따로 참고할 것.

> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 주봉 API에서 직접 수집한 것이다. **대조 결과**: 2026-08-14 종가 $418.80은 [`06_valuation.md`](./06_valuation.md)·[`09_technical_daily.md`](./09_technical_daily.md)에 인용된 값과 일치.
>
> ⚠️ 2026-07-01 Mobility Global(MBGL) 분사 관련 데이터 제공처(Yahoo Finance)의 "분할(split) 비율 1057:1000" 메타데이터는 실제 주식분할이 아니다 — 자세한 확인 내용은 [`09_technical_daily.md`](./09_technical_daily.md) 상단 ⚠️ 참고. 5년 구간 안에는 이 외의 실제 주식분할 이력이 없다.
...
@media (prefers-color-scheme: dark) {
  .spgi-chart { --bg:#1a1a19; --grid:#2c2c2a; --axis:#383835; --ink:#ffffff; --ink2:#c3c2b7; --muted:#898781; --up:#0ca30c; --down:#e66767; --support:#3987e5; --resistance:#d95926; --ref:#898781; }
}
...
> 5년 구간 최고가 $547.82(2025년 8월 부근 주봉 기준)·최저가 $264.26(2022년 하반기)로, 상단·하단 레벨이 일봉(`09_technical_daily.md`, 1년)보다 훨씬 넓은 범위를 커버한다 — 2022년 금리인상 사이클의 약세장까지 포함하기 때문.

---

## 3. 관측된 특이 구간

다년 관점에서 가장 뚜렷한 두 구간은 (1) 2022년 금리인상기 약세장, (2) 2025년 8월 고점 이후 2026년의 두 차례 가이던스 실망발 조정이다. 후자의 개별 갭다운 상세(일자별 등락률·거래량)는 [`09_technical_daily.md`](./09_technical_daily.md) §3을 참고하고, 여기서는 주봉 관점의 큰 그림만 기록한다.
...
- **2025년 8월 고점 → 2026년 조정**: 2025년 8월 주봉 고점(약 $527, 장중 최고 $579.05) 이후 2026년 2월 4분기 실적발표(가이던스 실망)로 급락해 3월 저점 $384 부근까지 조정, 이후 7월 초 Mobility 분사를 계기로 반등했으나 7월 말 2차 가이던스 하향으로 다시 밀려 현재 $418.80 수준 — 고점 대비 약 −28%. 두 조정 모두 실적/가이던스 발표가 직접 계기였다는 공통점이 있다(`09_technical_daily.md` §3 참고).
