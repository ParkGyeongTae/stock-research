# EQT Corporation (이큐티) — 기술적 분석 (일봉 캔들차트 · 지지/저항)
...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API에서 직접 수집한 것이다(1년 일봉은 `04_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). 2026-08-20 종가 $53.89는 [`04_metrics.md`](./04_metrics.md)·[`06_valuation.md`](./06_valuation.md)에 인용된 값과 일치(둘 다 stockanalysis.com 종가 기준). 차트 우측 상단 "현재가" 라벨은 스크립트 실행 시점(2026-08-21 장중)의 값이라 종가 기준값과 소폭(약 $0.07) 차이가 있을 수 있음 — 장중 스냅샷과 종가를 혼동하지 말 것.
...
- 2026-07-21 장 마감 후 Q2 2026 실적 발표([`08_news.md`](./08_news.md) 로그 참고) — 매출·FCF가 시장 예상을 상회.
...
- **한계**:
    ...
    - 원자재 가격(헨리허브 스팟가스) 자체의 계절 변동성이 이 회사 주가 변동의 상당 부분을 설명한다 — §3의 갭업도 실적 서프라이즈지만 그 배경엔 계절적 가격 강세가 있었다(`04_metrics.md` B절 참고). 순수 기술적 패턴만으로 이 변동을 해석하면 원자재 사이클이라는 근본 동인을 놓칠 수 있다.
