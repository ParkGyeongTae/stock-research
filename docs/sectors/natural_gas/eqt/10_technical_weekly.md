...
??? note "이 차트의 데이터 출처와 대조 결과"
    ...
    - **대조 결과**: 2026-08-20은 주봉 마감일(금요일)이 아니라 직접 대조가 어려우나, 가장 가까운 주봉 종가(2026-08-21 기준 최근 주) **$53.87**은 [핵심 지표](./04_metrics.md)·[밸류에이션 / 적정주가](./06_valuation.md)의 2026-08-20 종가 $53.89와 오차 범위 내에서 일치한다. 이 차트는 원주가(배당 미반영)를 쓴다.

!!! warning "5년 구간 안에 이 회사의 사업 구조가 바뀌었다"
    2024.07 Equitrans Midstream 재인수로 EQT는 순수 E&P에서 수직계열화 기업으로 전환됐고, 그 대가로 발행주식수가 42% 급증했다(역사 / 주요 이벤트·핵심 지표 참고). **2024.07 이전(순수 E&P 시기)의 스윙 레벨은 지금과 다른 사업·자본구조에서 형성된 것**이므로, 아래 2. 지지선 / 저항선 요약의 하위 레벨(S2 $38·S3 $35)은 지지선이 아니라 참고선으로 읽어야 한다.
...
> 레벨 개수는 5개(스크립트 기본값 3개에서 확대) — 위 "5년 구간 안에 이 회사의 사업 구조가 바뀌었다" 경고에서 밝힌 대로 합병 전후 자본구조 단절 때문에 하위 2개 레벨(S2·S3)은 참고선에 가깝다는 점을 표에 남기기 위해 확대했다(4. 방법론 · 한계에 반영).
