...
??? note "이 차트의 데이터 출처와 대조 결과"
    - **출처**: 이 차트 작성을 위해 주봉 API에서 직접 수집한 값이라 [핵심 지표](./04_metrics.md)의 원자료 표와는 계보가 다르다.
    - **대조 결과**: 가장 가까운 주봉 종가(2026-08-21 기준 최근 주) **$70.60**은 [핵심 지표](./04_metrics.md)·[밸류에이션 / 적정주가](./06_valuation.md)의 2026-08-21 종가 $70.13과 약 0.7% 차이가 있다 — [기술적 분석 — 일봉·1년](./09_technical_daily.md)에서 설명한 것과 같은 통상적인 데이터 제공처 간 오차 범위다. 이 차트는 원주가(배당 미반영)를 쓴다.

!!! warning "5년 구간 안에 이 회사의 사업 구성이 바뀌었다"
    EQT의 Equitrans 재인수처럼 자본구조가 단절적으로 바뀐 M&A는 이 5년(2021-08~2026-08) 안에 없었다([핵심 지표](./04_metrics.md) 상단 참고) — 발행주식수도 거의 변하지 않았다. 다만 사업 구성 자체는 점진적으로 달라졌다: 2025.07 CEO 교체, Power Innovation(데이터센터向 전력) 신사업 등장, 성장capex 사이클 진입([역사 / 주요 이벤트](./02_history.md)). **5년 전에 형성된 하단 지지 레벨(S3 등)은 순수 파이프라인 사업 시기의 가격대**라는 점을 감안해서 볼 것.
...
- 이 구간은 역사 / 주요 이벤트에서 확인한 Power Innovation(데이터센터向 전력) 신사업 부상·잇단 볼트온 M&A(Rimrock·Saber Midstream 등) 시기와 겹친다 — 밸류에이션 / 적정주가 2. 최근 3개년 — 적정주가 vs 실제주가에서도 같은 기간 PER·PBR 배수가 꾸준히 재평가(re-rating)돼온 것으로 확인된다.
...
- **생성**: `scripts/gen_technical_chart.py WMB --name "Williams Companies" --interval 1wk --close-on 2026-08-21` — 레벨 개수는 기본값(3) 그대로 사용, 위 2. 지지선 / 저항선 요약 각주처럼 저항 1개·지지 3개로 비대칭 산출.
- **한계**:
