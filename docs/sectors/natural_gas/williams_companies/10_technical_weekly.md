...
> ⚠️ 이 문서의 가격 데이터는 핵심 지표의 원자료 표와는 별도로, 이 차트 작성을 위해 주봉 API에서 직접 수집한 것이다. 가장 가까운 주봉 종가(2026-08-21 기준 최근 주) $70.60은 [핵심 지표](./04_metrics.md)·[밸류에이션 / 적정주가](./06_valuation.md)의 2026-08-21 종가 $70.13과 약 0.7% 차이가 있다 — [기술적 분석 — 일봉·1년](./09_technical_daily.md)에서 설명한 것과 같은 통상적인 데이터 제공처 간 오차 범위.
>
> ⚠️ 5년 조사 기간(2021-08~2026-08) 동안 EQT처럼 자본구조가 단절적으로 바뀐 M&A는 없었다(핵심 지표 상단 각주 참고) — 다만 2025.07 CEO 교체, Power Innovation(데이터센터向 전력) 신사업 등장 등 사업 구성 자체는 이 기간 동안 점진적으로 바뀌어왔다(역사 / 주요 이벤트). 5년 전 형성된 하단 지지 레벨(S3 등)은 순수 파이프라인 사업 시기의 가격대라는 점을 감안해서 볼 것.
