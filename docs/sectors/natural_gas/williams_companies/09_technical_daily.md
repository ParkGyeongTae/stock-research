...
??? note "이 차트의 데이터 출처와 대조 결과"
    - **출처**: 이 차트 작성을 위해 일봉 API에서 직접 수집한 값이라 [핵심 지표](./04_metrics.md)의 원자료 표와는 계보가 다르다(1년 일봉은 핵심 지표가 다루는 범위 밖이라 단일 출처 규칙의 예외다).
    - **대조 결과**: 스크립트가 수집한 2026-08-21 종가 **$70.54**는 [핵심 지표](./04_metrics.md)·[밸류에이션 / 적정주가](./06_valuation.md)에 인용된 값($70.13, Yahoo Finance 별도 조회)과 약 0.6% 차이가 있다 — 같은 날짜라도 데이터 제공처·조회 시점에 따라 종가가 소폭 갈리는 통상적 오차 범위이며, 방향성 판단에 영향을 줄 정도는 아니다. 이 차트는 원주가(배당 미반영)를 쓰므로 같은 기간 수정주가와는 배당만큼 차이가 난다.
