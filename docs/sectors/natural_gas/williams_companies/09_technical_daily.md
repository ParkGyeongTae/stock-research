# The Williams Companies (윌리엄스) — 기술적 분석 (일봉 캔들차트 · 지지/저항)
...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API에서 직접 수집한 것이다(1년 일봉은 `04_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). 스크립트가 수집한 2026-08-21 종가는 $70.54로, [`04_metrics.md`](./04_metrics.md)·[`06_valuation.md`](./06_valuation.md)에 인용된 값($70.13, Yahoo Finance 별도 조회)과 약 0.6% 차이가 있다 — 같은 날짜라도 데이터 제공처·조회 시점에 따라 종가가 소폭 갈리는 통상적 오차 범위이며, 방향성 판단에 영향을 줄 정도는 아니다.
...
- 2026-02-10 장중 FY2025 실적(Adjusted EBITDA $7.75B, 13년 연속 증가)과 2026 Analyst Day를 통해 2026년 가이던스(Adjusted EBITDA 중간값 $8.2B)·배당 5% 인상을 동시 발표([`08_news.md`](./08_news.md) 로그 참고).
...
- **한계**:
    ...
    - 조사 기간(2025-08~2026-08) 중 주식분할·대규모 유상증자는 없었다(`04_metrics.md` 상단 각주 참고).
    - 최근 1년 주가는 $56(2025-09) → $80(2026-05) → $70(2026-08)의 큰 사이클을 그렸다 — 이는 `05_financials.md`·`07_investment.md`에서 다룬 펀더멘털 재평가(성장 스토리에 대한 시장 신뢰 변화)와 맞물려 있을 가능성이 크며, 순수 기술적 패턴만으로는 이 변동의 근본 동인을 설명하지 못한다.
