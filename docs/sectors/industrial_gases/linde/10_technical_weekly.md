...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 Yahoo Finance 주봉 API에서 직접 수집했다(5년 주봉은 `04_metrics.md`가 다루는 범위 밖). 두 문서에서 겹치는 시점의 종가를 대조한 결과: 2026-08-14 종가 $482.74는 [`09_technical.md`](./09_technical.md)·`04_metrics.md`·`06_valuation.md`에서 인용한 stockanalysis.com 기준값과 정확히 일치한다.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| R1 | $516 | 2 | 2026년 2월·4월(2026-02-23·2026-04-20/27) 스윙 고점대 — `09_technical.md`의 R1($507)과 근접한 상위 밴드 |
...
| 참고선 | $548 | — | 5년 최고(2026-07, 사상 최고가 $548.20). 2026-07-31 Q2 실적 발표 갭다운(`09_technical.md` §3) 이후 가격대가 재설정되며 현재가와 12% 이상 괴리돼 있어 근시일 저항으로 보지 않음 |
...
- 이 조정은 회사 고유 이슈가 아니라 거시 금리 환경에 따른 밸류에이션 디레이팅 성격이 강하다. `09_technical.md` §3의 2026-07-31 실적발표 갭다운(회사 고유 이벤트)과는 원인이 다르므로 혼동하지 말 것.
...
- **한계**:
    ...
    - `09_technical.md` §3의 2026-07-31 실적발표 갭다운처럼 뉴스 이벤트로 인한 불연속 구간은 통상적인 지지/저항 해석이 적용되기 어렵다 — 참고선($548)을 근시일 저항으로 취급하지 않은 이유이기도 하다.
