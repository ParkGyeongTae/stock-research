...
> ⚠️ 이 문서의 가격 데이터는 `05_metrics.md`의 원자료 표와는 별도로, Yahoo Finance 일봉 API에서 직접 수집했다(1년 일봉은 `05_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). **겹치는 시점의 종가 대조**: 2026-08-14 종가 $482.74는 `05_metrics.md` A.2·`06_valuation.md`에서 인용한 stockanalysis.com 기준값($482.74)과 정확히 일치한다. 52주 최고 $548.20 역시 두 문서에서 동일하게 인용한 값과 일치.
...
- **데이터**: Yahoo Finance 일봉 OHLCV(Open/High/Low/Close/Volume), 251개 거래일, 2025-08-15~2026-08-14. 수집 시점: 2026-08-15. 원주가(과거 분할은 소급 반영, 배당은 미반영) — 최근 3년 내 주식분할 없음(`05_metrics.md` 상단 각주와 일치).
