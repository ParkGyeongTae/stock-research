...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API(Yahoo Finance)에서 직접 수집한 것이다(1년 일봉은 `04_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). 겹치는 시점의 종가를 대조한 결과: **2026-08-13 종가 $323.77**은 [`06_valuation.md`](./06_valuation.md) §6에 인용된 stockanalysis.com 값과 정확히 일치. 두 문서 모두 수정주가(배당·분할 미반영, Cadence는 표 기간 내 분할 이력 없음) 기준이라 대조에 문제가 없다.
