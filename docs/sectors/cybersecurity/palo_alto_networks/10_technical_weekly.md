...
> 레벨 개수는 3개(스크립트 기본값)로 고정하지 말 것 — 유효한 클러스터가 2개면 2개만 쓰고, 억지로 채우지 않는다. `--levels`로 늘리거나 줄였으면 그 사유를 4. 방법론 · 한계에 남긴다. 터치 2회 미만인데도 의미가 뚜렷해 `--force-level`로 넣은 레벨은 비고에 사유를 반드시 적을 것(예: 2022년 저점).
