...
> ⚠️ 이 문서의 가격 데이터는 핵심 지표의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API에서 직접 수집한 것이다(1년 일봉은 핵심 지표가 다루는 범위 밖이라 단일 출처 규칙의 예외). 대신 **두 문서에서 겹치는 시점의 종가를 반드시 대조하고 그 결과를 여기 적을 것** — `2026-08-14` 종가 `$384.27`은 [핵심 지표](./04_metrics.md)·[밸류에이션 / 적정주가](./06_valuation.md)에 인용된 stockanalysis.com 값과 일치 확인함.

> ⚠️ 상장 후 거래일이 6개월 미만이거나 유동성이 극히 얕은 종목은 스윙 클러스터가 표본 부족으로 무의미해진다 — 그럴 땐 이 문서를 만들지 말거나, 기간을 실제 거래 구간으로 줄이고 4. 방법론 · 한계에 그 사실을 남길 것.
...
- 2026-06-02 Q3 FY2026 실적발표([최근 뉴스 / 이슈](./08_news.md) 로그 참고) — 매출 31% 급증(CyberArk·Chronosphere 편입 효과)과 GAAP 영업손실이 동시에 나왔음에도, NGS ARR +60%·RPO +36%·FY2026 가이던스 상향이 부각되며 주가는 오히려 급등했다.
...
- 이 사건 이후 거래 레짐이 완전히 달라졌다 — 2025-09~2026-04의 박스권(S2 $180 부근)은 더 이상 근시일 지지로 기능하지 않는다고 보고 위 2. 지지선 / 저항선 요약에서 지지선이 아니라 사실상 "과거 레짐"으로만 취급했다(별도 참고선으로 격상하지 않은 이유: 현재가와 너무 멀어 실질적 지지선 역할을 하기 어렵다고 판단, 4. 방법론 · 한계 참고).
...
- **한계**:
    ...
    - 2026-02 CyberArk 인수 종결과 2026-06 Q3 실적발표 전후로 거래 레짐이 크게 바뀌어(3. 관측된 특이 구간 — 2026-06-02 Q3 FY2026 실적발표 이후 급등), 인수 종결 이전(~2026-04)의 스윙 레벨이 이후에도 유효한 지지/저항으로 작동할지는 불확실 — 위 2. 지지선 / 저항선 요약 S2·S3가 정확히 이 "과거 레짐" 구간에 해당한다.
