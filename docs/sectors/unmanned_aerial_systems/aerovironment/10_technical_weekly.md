...
- 다만 이 사건은 **가격보다 주당 지표(BPS·EPS·발행주식수)의 연속성을 깨는 자본구조적 이벤트**라는 점에서 별도로 기록한다 — [핵심 지표](./04_metrics.md) 상단 각주가 지적하듯 FY2025→FY2026 BPS·PBR 비교는 이 단절을 감안해서 읽어야 한다. 이 문서의 가격 패턴(지지·저항 레벨)에는 큰 영향을 남기지 않았지만, 펀더멘털 지표를 이 시점 전후로 비교할 때는 §3-A와 달리 "가격 차트로는 안 보이는 단절"이라는 점을 유의할 것.
