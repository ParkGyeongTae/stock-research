...
> ⚠️ 이 문서의 가격 데이터는 핵심 지표의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API(Yahoo Finance)에서 직접 수집한 것이다(1년 일봉은 핵심 지표가 다루는 범위 밖이라 단일 출처 규칙의 예외). 대신 겹치는 시점의 종가를 대조한 결과: `2026-04-30`(FY2026 회계연도 말) 종가 $195.02는 [핵심 지표](./04_metrics.md) A.2·[밸류에이션 / 적정주가](./06_valuation.md) §2에 인용된 값과 **일치**한다. 이 문서의 최신 종가($192.81, 2026-08-14)는 개요·밸류에이션 / 적정주가가 인용하는 기준일(2026-08-13, $189.43)보다 하루 늦은 시점의 값으로, 하루 사이 +1.78% 상승한 결과다 — 오차가 아니라 수집 시점 차이임을 밝혀둔다.
