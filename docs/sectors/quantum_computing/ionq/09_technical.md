# IonQ (아이온큐) — 기술적 분석 (일봉 캔들차트 · 지지/저항)
...
> ⚠️ 이 문서의 가격 데이터는 `04_metrics.md`의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API에서 직접 수집한 것이다(1년 일봉은 `04_metrics.md`가 다루는 범위 밖이라 단일 출처 규칙의 예외). 두 문서에서 겹치는 시점의 종가를 대조한 결과: `2025-12-31` 종가 $44.87, `2026-08-14` 종가 $46.26 — 둘 다 [`04_metrics.md`](./04_metrics.md) A.2·[`06_valuation.md`](./06_valuation.md) 상단에 인용된 값과 일치(주식분할 이력 없어 수정주가 이슈 없음).
...
@media (prefers-color-scheme: dark) {
  .ionq-chart { --bg:#1a1a19; --grid:#2c2c2a; --axis:#383835; --ink:#ffffff; --ink2:#c3c2b7; --muted:#898781; --up:#0ca30c; --down:#e66767; --support:#3987e5; --resistance:#d95926; --ref:#898781; }
}
...
- **한계**:
    ...
    - 이 종목은 최근 1년 변동성이 극히 커(고점 대비 저점 -70%) 스윙 레벨 자체의 "지지/저항"으로서의 의미가 안정된 대형주보다 약할 수 있다 — §3에서 보듯 국면 자체가 몇 달 단위로 크게 바뀌는 종목이라는 점을 감안할 것.
