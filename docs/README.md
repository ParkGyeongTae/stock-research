...
| `05_metrics.md` | 최근 3개년+올해(연간) / 최근 6개 분기의 매출·영업이익·PER·PBR·순부채·EV·유동비율·부채비율·FCF·SBC·배당(DPS) 등 **원자료 수치표** (평균·중앙값 비교용). C절에 그 사업 고유 지표(backlog·ARR·거래대금 등)를 둔다(아래 참고). 회사 관련 모든 문서가 참조하는 단일 출처(source of truth) | 선택 |
...
*작성일: 2026-08-01 (최종 수정: 2026-08-15 — `.pages` nav 등록 규칙·로컬 빌드 확인 방법·문서 간 링크 규칙 추가, "알려진 예외" 기준 명시. 사업 고유 지표(`05_metrics.md` C절) 규칙 신설과 기존 문서 미적용 현황 등록, `meta/` 내부의 성격 구분(읽는 문서 vs 스캐폴딩) 명시, `09_technical.md`(기술적 분석) 템플릿 추가에 따른 폴더 구조·파일 표 갱신. AGENTS.md "초안을 받으면 먼저 훑는 것"이 이 문서의 "숫자를 다룰 때 반드시 지키는 것"을 참조하도록 정리 — 규칙 본문의 단일 출처는 이 절)*
