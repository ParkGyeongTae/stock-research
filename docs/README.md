...
- 문서 맨 아래에 `*작성일: YYYY-MM-DD*` 표기 — 기존 문서를 고쳤으면 `(최종 수정: YYYY-MM-DD)`를 덧붙이고, 인용된 수치를 갱신했으면 그 값을 인용하는 다른 문서(`04_financials.md`·`06_valuation.md`·섹터 `01_comparison.md`, 종가가 겹치면 `09_technical.md`도)도 함께 확인
...
*작성일: 2026-08-01 (최종 수정: 2026-08-15 — `.pages` nav 등록 규칙·로컬 빌드 확인 방법·문서 간 링크 규칙 추가, "알려진 예외" 기준 명시. 사업 고유 지표(`05_metrics.md` C절) 규칙 신설과 기존 문서 미적용 현황 등록, `meta/` 내부의 성격 구분(읽는 문서 vs 스캐폴딩) 명시, `09_technical.md`(기술적 분석) 템플릿 추가에 따른 폴더 구조·파일 표 갱신. AGENTS.md "초안을 받으면 먼저 훑는 것"이 이 문서의 "숫자를 다룰 때 반드시 지키는 것"을 참조하도록 정리 — 규칙 본문의 단일 출처는 이 절. `05_metrics.md` 파일표 설명에 D절(판단 메모) 명시, 인용처 추적 대상에 `09_technical.md`(종가 대조) 추가)*
