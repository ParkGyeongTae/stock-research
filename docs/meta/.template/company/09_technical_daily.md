...
> ⚠️ 이 문서의 가격 데이터는 핵심 지표의 원자료 표와는 별도로, 이 차트 작성을 위해 일봉 API에서 직접 수집한 것이다(1년 일봉은 핵심 지표가 다루는 범위 밖이라 단일 출처 규칙의 예외). 대신 **두 문서에서 겹치는 시점의 종가를 반드시 대조하고 그 결과를 여기 적을 것** — 예: `<YYYY-MM-DD>` 종가 `$<X>`는 [밸류에이션 / 적정주가](./06_valuation.md)에 인용된 `<출처>` 값과 일치. 어긋나면 어느 쪽이 수정주가(배당·분할 반영)인지부터 확인한다.

> ⚠️ 상장 후 거래일이 6개월 미만이거나 유동성이 극히 얕은 종목은 스윙 클러스터가 표본 부족으로 무의미해진다 — 이 문서 자체를 생략하지 말고, 기간을 실제 거래 구간으로 줄여서 만들고 표본이 부족하다는 사실을 4. 방법론 · 한계에 남길 것.
