...
>
> **이 문서는 과거 가격 패턴에 대한 객관적 서술이며, 코인 시세 전망이나 투자 판단이 아니다.** 이 레포는 개별 주식 리서치가 중심이고, BTC 자체를 투자 대상으로 다루지 않는다 — [`concepts/macroeconomics.md`](../../concepts/macroeconomics.md) "거시를 어디까지 쓸 것인가"와 같은 원칙을 적용해, `digital_asset_finance` 섹터 회사의 실적을 이해하기 위한 배경 자료로만 쓴다.
>
> ⚠️ BTC/USD는 **24시간 365일 거래**된다 — 다른 macro 문서(주식시장 개장일 기준)와 달리 "주 마지막 거래일"이 주말(예: 일요일 UTC 자정)일 수 있고, 거래량 단위도 주식과 달라 §3 수치의 "주평균 거래량"은 참고용으로만 볼 것.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| R3 | $125,328 | 2 | 2025-08-11·2025-10-06 |
| R2 | $108,692 | 2 | 2024-12-16·2025-01-20 |
| R1 | $72,848 | 2 | 2024-03-11·2024-05-20 |
| **현재가** | **$72,171.72** (2026-08-20 종가) | — | R1과 S1 사이 |
| S1 | $57,152 | 2 | 2024-04-29·2026-06-29 |
| S2 | $24,864 | 2 | 2023-06-12·2023-09-11 |

---

## 3. 방법론 · 한계

- **데이터**: Yahoo Finance 주봉 OHLCV(주간 시가/고가/저가/종가/거래량, 주 마지막 거래일 기준), 263개 주, 2021-08-16~2026-08-20. 수집 시점: 2026-08-20. BTC/USD 원자료(조정 없음, 24시간 시장이라 주 마지막 거래일 기준 종가)
- **스윙 포인트 탐지**: 각 주의 고가/저가가 전후 4주(총 9주 창) 내 최고/최저값과 같으면 스윙 고점/저점으로 분류.
- **클러스터링**: 스윙 포인트를 가격 오름차순으로 정렬한 뒤, 이미 만든 클러스터 중심과 ±2.5% 이내면 같은 클러스터로 합산하고 중심을 재계산. 터치 2회 이상만 표시(예외는 §2 비고).

<!-- 참고용 수치 (문서에는 필요한 것만 옮길 것) -->
<!-- 최근 5년 최고 126,198.07 / 최저 15,599.05 / y축 12,000.00~130,000.00 / 주평균 거래량 241,625,676,273주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-20 종가 $72,171.72 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 가격이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - BTC는 이 레포에서 다루는 다른 자산보다 변동성이 훨씬 크다 — 같은 ±2.5% 클러스터 허용오차를 그대로 적용했지만(다른 09/10_technical_*.md·macro 문서와 비교 가능성을 유지하기 위해), 실제로 "의미 있는 레벨"과 "노이즈"의 경계는 다른 자산보다 더 넓게 잡아야 할 수 있다.
    - 24시간 거래·규제 공백기가 섞여 있어 전통 자산과 같은 거래일 개념이 정확히 대응하지 않는다(위 ⚠️ 참고).
...
uv run python scripts/gen_technical_chart.py "BTC-USD" --interval 1wk --emit dates
```
