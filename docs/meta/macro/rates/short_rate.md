# 13주 단기금리 (US 3M T-Bill)

!!! note ""
    최근 5년 미 국채 13주(3개월) 할인율(`^IRX`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. 사실상 연준 기준금리의 시장 프록시라, 10년물 국채금리와 함께 봐서 **장단기 금리차(수익률곡선)** 국면을 확인할 때 인용한다.
...
- **왜 이런 신호로 읽히나**: 연준이 실제로 통제하는 것은 익일물(overnight) 기준금리(연방기금금리)이고, 13주물은 만기가 짧아 기대가설 성분이 거의 전부다 — 기간 프리미엄이 사실상 0에 가까워, 향후 3개월간 시장이 가격에 반영한 인상·인하 확률을 가장 직접적으로 드러낸다.
