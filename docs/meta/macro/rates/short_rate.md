...
- [`treasury_10y.md`](./treasury_10y.md)와의 스프레드(장단기 금리차)가 단독 수준보다 더 자주 인용된다 — [`concepts/macroeconomics.md`](../../concepts/macroeconomics.md) "수익률곡선" 참고.
...
uv run python scripts/gen_technical_chart.py "^IRX" --name "13주 단기금리" --interval 1wk \
  ...
  --adj-note "13주 국채 할인율 원자료(조정 없음)" --close-on <YYYY-MM-DD> --emit chart
```

[`treasury_10y.md`](./treasury_10y.md)도 같은 `--close-on` 날짜로 함께 갱신하면 두 문서의 종가를 바로 대조해 장단기 스프레드를 확인할 수 있다. 커맨드 문법은 [`../../authoring-guide.md`](../../authoring-guide.md) "주가가 아닌 시계열에 쓰기" 참고.
