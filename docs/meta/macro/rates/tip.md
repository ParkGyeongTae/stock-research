...
- **상승**: TIPS 원금이 CPI에 연동돼 조정되므로, 명목채([`tlt.md`](./tlt.md)) 대비 상대 강도가 강해지면 기대인플레이션 상승 신호로 흔히 해석된다.
- **하락**: 명목채 대비 상대 강도가 약해지면 기대인플레이션 하락(또는 실질금리 상승) 신호로 흔히 해석된다.
- 정밀한 기대인플레이션(breakeven inflation) 수치가 아니다 — 실제 BEI는 만기가 일치하는 명목채·물가연동채 수익률을 빼서 계산한다(예: FRED `T10YIE`). 이 문서는 [`tlt.md`](./tlt.md)와의 방향을 정성적으로 비교하는 용도로만 쓴다.
