# 물가연동국채 ETF (TIP) — 기술적 참고 (주봉 5년)

> 최근 5년 iShares 물가연동국채(TIPS) ETF(`TIP`) 주봉 가격 흐름을 지지선·저항선과 함께 정리한 참고 자료. TIPS는 원금이 CPI에 연동돼 조정되는 국채라, [`tlt.md`](./tlt.md)(명목 장기국채) 대비 상대 강도를 보면 시장이 반영하는 **기대인플레이션의 대략적인 방향**을 가늠하는 실마리가 된다.
>
> ⚠️ **정밀한 기대인플레이션(breakeven inflation) 수치가 아니다** — 실제 BEI는 만기가 일치하는 명목채·물가연동채 수익률을 빼서 계산하며(예: FRED `T10YIE`), 이 문서는 그 계산을 하지 않는다. 이 문서와 [`tlt.md`](./tlt.md)의 방향을 정성적으로 비교하는 용도로만 쓴다.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| R2 | $130 | 2 | 2021-11-08·2022-03-07 |
| R1 | $110 | 13 | 2022-11-28·2023-01-30·2023-04-03·2023-12-11·2024-01-29·2024-03-04·2024-09-16·2024-12-02·2025-03-31·2025-09-15·2025-10-20·2026-02-23·2026-04-27 |
| **현재가** | **$107.51** (2026-08-19 종가) | — | R1과 S1 사이 |
| S1 | $106 | 10 | 2022-09-26·2022-12-26·2023-03-06·2024-02-12·2024-04-29·2024-07-01·2024-11-11·2024-12-16·2025-04-07·2025-05-19 |
...
<!-- 최근 5년 최고 131.37 / 최저 101.74 / y축 100.50~132.50 / 주평균 거래량 14,969,371주 -->
<!-- ⚠️ 기간 내 배당 48회 — 원주가라 배당 미반영임을 §4에 명시 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-19 종가 $107.51 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 가격이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - **가격 기준(price return)이지 총수익률(total return)이 아니다** — TIP은 매달 이자 분배금을 지급하는 ETF이고, 이 차트의 종가는 분배금이 재투자됐다고 가정하지 않은 가격만 반영한다. 게다가 TIPS 자체의 원금 인플레이션 조정분도 이 가격 하나로는 분해해서 볼 수 없다.
    - 위 ⚠️에 적었듯 정밀한 기대인플레이션 계산이 아니다 — 그런 수치가 필요하면 FRED 등 원출처를 직접 확인한다.
...
```bash
uv run python scripts/gen_technical_chart.py "TIP" --name "물가연동국채 ETF" --interval 1wk \
  --adj-note "ETF 원자료(가격 기준, 분배금 재투자 미반영 — 총수익률 아님)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "TIP" --interval 1wk --emit dates
```
