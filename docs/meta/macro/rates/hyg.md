...
- **왜 이런 신호로 읽히나**: 채권 가격은 미래 쿠폰·원금의 현재가치이고, 할인율은 "무위험금리 + 신용스프레드"로 분해된다. 무위험금리가 크게 안 움직인 구간에서 HYG 가격이 변했다면 스프레드(신용 프리미엄)가 움직였다는 신호로 읽을 수 있다 — 다만 실제로는 무위험금리도 함께 움직이므로 순수 스프레드로 단정하지 않는다.
