# 미국 10년물 국채금리 (US 10Y Treasury Yield) — 기술적 참고 (주봉 5년)
...
>
> **이 문서는 과거 금리 패턴에 대한 객관적 서술이며, 금리 전망이나 투자 판단이 아니다.** 금리가 개별 종목 밸류에이션에 닿는 두 개의 경로(할인율/실적)는 [`concepts/macroeconomics.md`](../../concepts/macroeconomics.md) "금리가 주가에 닿는 두 개의 경로"를 따로 참고할 것 — 이 문서는 그 경로 설명을 다시 옮기지 않고 가격 데이터만 다룬다.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| R1 | 4.74% | 3 | 2024-04-22·2025-01-13·2026-05-18 |
| **현재가** | **4.65%** (2026-08-17 종가) | — | R1과 S1 사이 |
| S1 | 4.17% | 2 | 2024-12-02·2025-06-30 |
| S2 | 3.98% | 3 | 2024-03-04·2025-10-20·2026-02-23 |
| S3 | 3.83% | 3 | 2023-12-25·2024-01-29·2025-03-31 |
...
<!-- 최근 5년 최고 5.00 / 최저 1.23 / y축 1.10~5.15 / 주평균 거래량 0주 -->
...
- **한계**:
    ...
    - 국채금리는 연준 정책·인플레이션 기대·재정정책 등 여러 요인이 겹쳐 움직인다 — 이 차트만으로 방향을 예단하지 않는다. 경로에 대한 설명은 [`concepts/macroeconomics.md`](../../concepts/macroeconomics.md) "연준과 금리 결정"·"수익률곡선" 절 참고.
    - 윈도우(4주)·허용오차(±2.5%) 값을 바꾸면 레벨과 터치 횟수가 달라진다. 여기 쓰인 값은 최적화된 값이 아니라 09/10_technical_*.md와 회사 간 비교가 가능하도록 고정한 공통 파라미터다.
    - `^TNX`는 종가 기준 수익률이며, 회사 문서의 DCF에서 "조회 시점" 무위험이자율로 인용할 때는 이 문서의 마지막 종가가 아니라 **DCF 작성 시점의 최신 값**을 다시 확인해 쓸 것 — 이 문서는 주기적으로만 갱신된다.
...
uv run python scripts/gen_technical_chart.py "^TNX" --interval 1wk --emit dates
```

`--close-on`에는 이 값을 DCF 무위험이자율 근거로 인용하는 회사 문서(`06_valuation.md`)와 대조할 날짜를 넣는다. `--emit dates` 출력은 레벨(R1/S1 등)별 날짜 목록이며, 그 시기에 무슨 일이 있었는지(국면 해석)는 자동으로 나오지 않으니 직접 확인해서 덧붙인다.
