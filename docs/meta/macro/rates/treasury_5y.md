...
!!! note ""
    최근 5년 미 국채 5년물 수익률(`^FVX`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. 13주 단기금리·10년물·30년물 국채금리와 함께 **수익률곡선의 중간 구간**을 채운다.

    **왜 별도로 두는가**: 수익률곡선 이론에 따르면 만기가 짧을수록 정책금리 기대를, 길수록 장기 성장·물가 기대를 반영한다 — 5년물은 그 중간 지점으로, 단기(13주)만큼 정책에 즉각 반응하지도, 장기(10년·30년)만큼 먼 미래 기대에만 좌우되지도 않는다.
...
- **왜 이런 신호로 읽히나**: 국채 수익률은 대략 "해당 만기 동안 시장이 예상하는 평균 단기금리 경로(기대가설)" + "기간 프리미엄"의 합으로 분해된다. 5년은 통화정책 한 사이클(인상 → 고점 → 인하)이 걸리는 기간과 맞물려, 시장이 보는 **다음 완화 사이클의 종착점(터미널 금리)** 기대를 비교적 잘 드러내는 만기로 흔히 쓰인다.
