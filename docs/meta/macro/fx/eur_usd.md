...
!!! note ""
    최근 5년 유로/달러 환율(1유로당 USD, `EURUSD=X`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. `dxy.md`(달러인덱스)의 최대 비중(약 58%) 구성 통화쌍이 바로 이것이라, DXY 움직임의 대부분은 사실상 이 환율이 주도한다 — DXY가 왜 움직였는지 더 세밀하게 보고 싶을 때 이 문서를 함께 본다.
