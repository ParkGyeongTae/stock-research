...
!!! note ""
    최근 5년 원/달러(1달러당 원화, `KRW=X`) 주봉 가격 흐름을 지지선·저항선과 함께 정리한 참고 자료. 해외 매출 비중이 큰 회사의 환율 민감도를 Bear/Bull Case에서 다룰 때, 혹은 상수통화(constant currency) 성장률과 보고 성장률의 괴리를 설명할 때 이 문서를 인용한다.
