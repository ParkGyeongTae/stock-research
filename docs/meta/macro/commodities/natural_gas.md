...
>
> ⚠️ **현재 커버리지와의 연관성**: 이 레포가 지금 다루는 섹터는 천연가스 가격에 직접 노출된 회사가 없다. 에너지·유틸리티 섹터를 다루게 되면 그 회사 문서에서 이 차트를 우선 인용한다.
>
...
각 레벨은 "전후 4주 내 최고/최저인 스윙 포인트"를 가격 기준 ±2.5% 이내로 묶은 클러스터다. 터치 횟수는 그 클러스터에 포함된 스윙 포인트 개수(강도 근사치)를 뜻하며, 미래 지지/저항을 보장하지 않는다(§3 한계 참고). 단위는 MMBtu(백만 영국열량단위)당 USD다.

| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| R2 | $3.44 | 3 | 2024-01-08·2026-03-09·2026-06-22 |
| R1 | $3.02 | 3 | 2023-02-27·2023-08-07·2024-09-30 |
| **현재가** | **$2.76** (2026-08-20 종가) | — | R1과 S1 사이 |
| S1 | $2.45 | 2 | 2023-08-21·2026-04-27 |
| S2 | $2.22 | 2 | 2023-12-11·2024-10-28 |
| S3 | $1.96 | 2 | 2023-02-20·2023-03-27 |
...
<!-- 최근 5년 최고 10.03 / 최저 1.48 / y축 1.20~10.40 / 주평균 거래량 707,185주 -->
...
- **한계**:
    ...
    - **연속월물(continuous contract) 특성** — `NG=F`는 최근월물을 이어붙인 연속 시계열이라, 롤오버 시점에 실제 현물가와 무관한 가격 갭이 생길 수 있다.
    - **계절성이 매우 강하다** — 겨울철 난방 수요·재고 수준에 따라 단기 변동성이 원유보다 훨씬 크다. 특정 레벨을 계절 요인과 분리해서 보지 않으면 오독하기 쉽다.
...
uv run python scripts/gen_technical_chart.py "NG=F" --interval 1wk --emit dates
```
