...
> 최근 5년 닛케이225 지수(일본, `^N225`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. [`jpy_usd.md`](../fx/jpy_usd.md)(엔/달러)와 함께 보면 일본 시장·엔화 국면을 종합적으로 확인할 수 있다 — 엔 약세는 대개 일본 수출기업 실적 기대를 밀어올려 닛케이 강세와 같이 나타나는 경우가 많다.
>
...
각 레벨은 "전후 4주 내 최고/최저인 스윙 포인트"를 지수 기준 ±2.5% 이내로 묶은 클러스터다. 터치 횟수는 그 클러스터에 포함된 스윙 포인트 개수(강도 근사치)를 뜻하며, 미래 지지/저항을 보장하지 않는다(§3 한계 참고). 단위는 무차원 지수(포인트, 엔화 표시 종목의 주가 평균이지만 관례상 엔 단위를 붙이지 않는다).

| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| **현재가** | **66,216.79** (2026-08-20 종가) | — | 기간 내 상단 저항 없음(신고가 구간) — 가장 가까운 지지는 S1 |
| S1 | 37,469 | 3 | 2024-04-15·2024-05-27·2025-01-13 |
| S2 | 30,928 | 4 | 2023-08-14·2023-10-02·2024-08-05·2025-04-07 |
| S3 | 27,172 | 3 | 2021-10-04·2021-11-29·2023-03-13 |
...
<!-- 최근 5년 최고 72,831.73 / 최저 24,681.74 / y축 23,000.00~74,500.00 / 주평균 거래량 506,344,487주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-19 종가 — 거래일 아님 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 지수 수준이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - 닛케이225도 다우존스처럼 주가 가중 지수라, 시가총액과 무관하게 주가 자체가 높은 종목의 움직임이 과대 반영된다.
...
```bash
uv run python scripts/gen_technical_chart.py "^N225" --name "닛케이225" --interval 1wk \
  --symbol "" --unit-label "지수" \
  --adj-note "지수 원자료(조정 없음)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "^N225" --interval 1wk --emit dates
```
