...
- **참고**: 닛케이225도 다우존스산업지수처럼 **가격가중** 방식이라 시가총액과 무관하게 주가가 높은 종목의 비중이 크다. 일본은행(BOJ)의 초완화 통화정책이 장기간 이어지며 엔화가 대표적인 캐리트레이드 조달 통화로 자리잡은 배경도, 엔화 약세가 닛케이 강세로 이어지는 경로를 강화해왔다.
