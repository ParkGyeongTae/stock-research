...
> 최근 5년 러셀2000 지수(미국 중소형주 2,000종목, `^RUT`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. [`sp500.md`](./sp500.md)·[`nasdaq.md`](./nasdaq.md)·[`sox.md`](./sox.md)가 전부 대형주 위주 지수인 것과 달리, 이 지수는 **미국 국내 경기에 더 민감한 중소형주**를 담아 다른 신호를 준다 — 대형주 지수는 오르는데 러셀2000이 약하면 시장 강세가 소수 대형주에 쏠려 있다는 신호로 읽히곤 한다.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| **현재가** | **3,032.94** (2026-08-19 종가) | — | 기간 내 상단 저항 없음(신고가 구간) — 가장 가까운 지지는 S1 |
| S1 | 2,152 | 3 | 2021-09-20·2025-01-13·2025-07-28 |
| S2 | 1,993 | 2 | 2024-06-17·2024-08-05 |
| S3 | 1,908 | 3 | 2022-02-21·2024-01-15·2024-04-15 |

---

## 3. 방법론 · 한계

- **데이터**: Yahoo Finance 주봉 OHLCV(주간 시가/고가/저가/종가/거래량, 주 마지막 거래일 기준), 263개 주, 2021-08-16~2026-08-19. 수집 시점: 2026-08-20. 지수 원자료(조정 없음)
- **스윙 포인트 탐지**: 각 주의 고가/저가가 전후 4주(총 9주 창) 내 최고/최저값과 같으면 스윙 고점/저점으로 분류.
- **클러스터링**: 스윙 포인트를 가격 오름차순으로 정렬한 뒤, 이미 만든 클러스터 중심과 ±2.5% 이내면 같은 클러스터로 합산하고 중심을 재계산. 터치 2회 이상만 표시(예외는 §2 비고).

<!-- 참고용 수치 (문서에는 필요한 것만 옮길 것) -->
<!-- 최근 5년 최고 3,069.71 / 최저 1,633.67 / y축 1,575.00~3,125.00 / 주평균 거래량 21,567,252,548주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-19 종가 3,032.94 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 지수 수준이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - 중소형주 지수 특성상 대형주 지수보다 금리 변화(특히 단기 조달금리)에 더 민감하게 반응하는 경향이 있다는 통념이 있으나, 이 차트 자체는 그 인과관계를 검증하지 않는다.
...
```bash
uv run python scripts/gen_technical_chart.py "^RUT" --name "러셀2000" --interval 1wk \
  --symbol "" --unit-label "지수" \
  --adj-note "지수 원자료(조정 없음)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "^RUT" --interval 1wk --emit dates
```
