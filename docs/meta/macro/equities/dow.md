# 다우존스산업지수 — 기술적 참고 (주봉 5년)

> 최근 5년 다우존스산업지수(우량주 30종목, `^DJI`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. 시가총액 가중이 아니라 **주가 가중** 방식이라 [`sp500.md`](./sp500.md)·[`nasdaq.md`](./nasdaq.md)와 계산 방식 자체가 다르다 — 종목 수가 적고(30개) 전통 산업재·금융 비중이 커서, 성장주 중심의 나스닥과는 다른 흐름을 보일 때가 있다.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| **현재가** | **53,463.05** (2026-08-19 종가) | — | 기간 내 상단 저항 없음(신고가 구간) — 가장 가까운 지지는 S1 |
| S1 | 45,413 | 3 | 2025-10-13·2025-11-17·2026-03-30 |
| S2 | 41,746 | 2 | 2024-11-04·2025-01-13 |
| S3 | 38,055 | 2 | 2024-04-15·2024-08-05 |
...
<!-- 최근 5년 최고 54,744.33 / 최저 28,660.94 / y축 27,500.00~56,000.00 / 주평균 거래량 1,981,225,115주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-19 종가 53,463.05 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 지수 수준이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - **주가 가중 지수**라 시가총액과 무관하게 주가 자체가 높은 종목(예: 특정 고가주)의 움직임이 지수에 과대 반영된다 — S&P 500·나스닥(시가총액 가중)과 같은 방식으로 해석하지 않는다.
...
```bash
uv run python scripts/gen_technical_chart.py "^DJI" --name "다우존스산업지수" --interval 1wk \
  --symbol "" --unit-label "지수" \
  --adj-note "지수 원자료(조정 없음)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "^DJI" --interval 1wk --emit dates
```
