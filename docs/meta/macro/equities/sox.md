...
!!! note ""
    최근 5년 필라델피아 반도체지수(`^SOX`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. 범용 거시지표가 아니라 반도체 설계·제조·장비 기업으로 구성된 **섹터 지수**다.

    **어떻게 쓰나**: 개별 회사 주가가 SOX 대비 초과 상승/하락하고 있다면, 그 회사 고유의 이슈(펀더멘털)인지 반도체 업황 전체(사이클)의 문제인지 구분하는 첫 단서가 된다. 개별 회사의 기술적 차트와 SOX 차트를 같은 기간으로 나란히 보는 방식을 권한다.
