...
>
> **이 문서는 과거 가격 패턴에 대한 객관적 서술이며, 지수 전망이나 투자 판단이 아니다.**
>
> 09/10_technical_*.md(개별 회사 주가 차트)와 같은 스크립트(`scripts/gen_technical_chart.py`)를 쓰지만, 이 문서는 회사 폴더가 아니라 `meta/`에 있다 — 특정 회사에는 종속되지 않지만 섹터에는 종속되는, 조금 특수한 경우다([`../../../README.md`](../../../README.md) "📁 폴더 구조" 참고).
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| **현재가** | **11,738.23** (2026-08-19 종가) | — | 기간 내 상단 저항 없음(신고가 구간) — 가장 가까운 지지는 S1 |
| S1 | 4,289 | 2 | 2024-04-15·2024-08-05 |
| S2 | 3,397 | 2 | 2023-08-14·2025-04-07 |
| S3 | 3,155 | 3 | 2021-10-11·2022-01-24·2023-10-30 |
...
<!-- 최근 5년 최고 14,655.29 / 최저 2,089.82 / y축 1,500.00~15,250.00 / 주평균 거래량 0주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-19 종가 11,738.23 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 지수 수준이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - SOX는 시가총액 가중 지수라 대형 회원사(엔비디아 등) 몇 곳의 움직임이 지수 전체를 좌우할 수 있다 — "반도체 업황 전체"로 곧장 일반화하지 말고, 지수 구성 상위 종목의 비중을 함께 확인한다.
...
```bash
uv run python scripts/gen_technical_chart.py "^SOX" --name "필라델피아 반도체지수" --interval 1wk \
  --symbol "" --unit-label "지수" \
  --adj-note "지수 원자료(조정 없음)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "^SOX" --interval 1wk --emit dates
```
