...
>
> **이 문서는 과거 변동성 패턴에 대한 객관적 서술이며, 시장 전망이나 투자 판단이 아니다.** VIX는 개별 종목처럼 방향성을 갖고 추세를 이어가기보다 **평균회귀(mean-reversion) 성향이 강한 지표**라, 지지선·저항선(추세 지속을 전제로 발전한 기법)을 그대로 적용하는 데는 다른 자산보다 한계가 크다 — §3에서 다시 짚는다.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| R3 | 23.38 | 2 | 2024-10-28·2026-06-08 |
| R2 | 22.70 | 3 | 2023-10-23·2025-01-27·2025-06-23 |
| R1 | 21.19 | 3 | 2023-05-01·2024-04-15·2026-07-27 |
| **현재가** | **15.23** (2026-08-20 종가) | — | R1과 S1 사이 |
| S1 | 12.65 | 5 | 2023-06-19·2023-07-24·2023-09-11·2024-03-18·2024-12-02 |
| S2 | 11.67 | 2 | 2023-12-11·2024-05-20 |

---

## 3. 방법론 · 한계

- **데이터**: Yahoo Finance 주봉 OHLCV(주간 시가/고가/저가/종가/거래량, 주 마지막 거래일 기준), 263개 주, 2021-08-16~2026-08-20. 수집 시점: 2026-08-20. VIX 지수 원자료(조정 없음)
- **스윙 포인트 탐지**: 각 주의 고가/저가가 전후 4주(총 9주 창) 내 최고/최저값과 같으면 스윙 고점/저점으로 분류.
- **클러스터링**: 스윙 포인트를 가격 오름차순으로 정렬한 뒤, 이미 만든 클러스터 중심과 ±2.5% 이내면 같은 클러스터로 합산하고 중심을 재계산. 터치 2회 이상만 표시(예외는 §2 비고).

<!-- 참고용 수치 (문서에는 필요한 것만 옮길 것) -->
<!-- 최근 5년 최고 65.73 / 최저 10.62 / y축 8.00~68.00 / 주평균 거래량 0주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-20 종가 15.23 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 VIX 수준이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - **평균회귀 성향** — VIX는 장기적으로 특정 레인지(대략 12~20 안팎)로 되돌아가려는 성향이 알려져 있다. 이 문서의 스윙 클러스터는 그 레인지의 경계를 보여줄 뿐, 추세 지속을 전제로 한 "저항 돌파 시 추가 상승" 같은 해석은 다른 자산보다 신뢰도가 낮다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - VIX 급등은 대개 특정 이벤트(실적 시즌, 지정학적 충격, 유동성 이벤트)에 짧게 반응하고 빠르게 되돌아간다 — 레벨 하나만으로 국면을 판단하지 않는다.
...
```bash
uv run python scripts/gen_technical_chart.py "^VIX" --name "VIX 변동성지수" --interval 1wk \
  --symbol "" --unit-label "pt" --decimals 2 \
  --adj-note "VIX 지수 원자료(조정 없음)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "^VIX" --interval 1wk --emit dates
```

`--decimals 2`는 필수에 가깝다 — VIX는 20 이상 구간에서도 1 미만 차이가 서로 다른 레벨을 가르는데, 스크립트의 기본 반올림 규칙(20 이상이면 정수 단위)은 주가 기준이라 이 지수엔 너무 거칠다.
