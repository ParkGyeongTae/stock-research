...
!!! note ""
    최근 5년 CBOE 변동성지수(S&P 500 옵션 내재변동성 기반, `^VIX`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. 개별 종목 밸류에이션이 아니라 **시장 전체의 위험선호도**를 보여주는 지표다 — 수치가 높을수록 옵션시장이 향후 변동성(공포심리)을 크게 반영하고 있다는 뜻이다.
