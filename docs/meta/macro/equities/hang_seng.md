...
> 최근 5년 항셍지수(홍콩거래소, `^HSI`) 주봉 흐름을 지지선·저항선과 함께 정리한 참고 자료. 중국 본토 규제·경기와 홍콩 금융시장 유동성이 겹쳐 반영되는 지수라, 중화권 리스크 센티먼트를 볼 때 [`nikkei225.md`](./nikkei225.md)와 함께 아시아 시장 배경으로 참고한다.
>
...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| R1 | 26,913 | 4 | 2021-10-25·2025-09-29·2025-11-10·2026-05-11 |
| **현재가** | **25,698.49** (2026-08-20 종가) | — | R1과 S1 사이 |
| S1 | 25,116 | 2 | 2025-10-13·2025-12-15 |
| S2 | 23,942 | 2 | 2021-10-04·2026-03-23 |
| S3 | 22,592 | 2 | 2021-12-20·2026-06-22 |
...
<!-- 최근 5년 최고 28,056.10 / 최저 14,597.31 / y축 14,000.00~28,500.00 / 주평균 거래량 13,109,624,335주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-20 종가 25,698.49 -->
...
- **한계**:
    ...
    - 중국 본토 규제 리스크·정책 이벤트에 급격히 반응하는 경우가 있다 — 이 차트만으로 방향을 예단하지 않는다.
...
```bash
uv run python scripts/gen_technical_chart.py "^HSI" --name "항셍지수" --interval 1wk \
  --symbol "" --unit-label "지수" \
  --adj-note "지수 원자료(조정 없음)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "^HSI" --interval 1wk --emit dates
```
