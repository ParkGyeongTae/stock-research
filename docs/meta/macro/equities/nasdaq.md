...
| 레벨 | 가격 | 터치 횟수 | 비고 |
|------|------|-----------|------|
| **현재가** | **26,331.09** (2026-08-19 종가) | — | 기간 내 상단 저항 없음(신고가 구간) — 가장 가까운 지지는 S1 |
| S1 | 24,703 | 2 | 2026-06-08·2026-07-27 |
| S2 | 12,550 | 2 | 2022-03-14·2023-10-23 |
| S3 | 10,148 | 2 | 2022-10-10·2022-12-26 |
...
<!-- 최근 5년 최고 27,190.21 / 최저 10,088.83 / y축 9,500.00~27,750.00 / 주평균 거래량 30,694,930,471주 -->
<!-- 04_metrics.md/06_valuation.md 대조용 종가 -->
<!-- 2026-08-19 종가 26,331.09 -->

- **한계**:
    - 후행적(과거 데이터 기반) 지표다. 특정 지수 수준이 지지·저항으로 "작동할 것"을 보장하지 않는다.
    - 이동평균·추세선 등 다른 기술적 지표는 포함하지 않았다 — 스윙 고점/저점 빈도만 반영한 단순 모델이다.
    - 시가총액 가중 지수라 상위 몇 개 대형 기술주의 비중이 매우 크다 — 지수 움직임을 "성장주 전체가 골고루 움직였다"로 곧장 해석하지 않는다.
...
```bash
uv run python scripts/gen_technical_chart.py "^IXIC" --name "나스닥종합지수" --interval 1wk \
  --symbol "" --unit-label "지수" \
  --adj-note "지수 원자료(조정 없음)" --close-on <YYYY-MM-DD>

# §2 표 '비고' 열(어느 시기의 스윙대인지)을 채울 날짜는 별도로 뽑는다 — 눈대중 금지
uv run python scripts/gen_technical_chart.py "^IXIC" --interval 1wk --emit dates
```
