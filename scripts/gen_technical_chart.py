...
한계 — 마지막 봉은 진행 중인 구간이다: 원자료를 range로 실행 시점까지 받아
그대로 그리며, 종료일을 고정하는 옵션은 없다(--close-on은 §4 데이터 블록에 특정일
종가를 덧붙일 뿐 시계열을 자르지 않는다). 주봉을 주중에 돌리면 마지막 봉의 종가·
고가·저가는 그 주가 끝나기 전 값이고, 다음 실행에서 달라진다. 그래서 차트에서 읽은
